...
from domains.gym_taxi.simulator.taxi_world import TaxiWorldSimulator, ACTIONS
from domains.gym_taxi.utils.spaces import Json
from domains.gym_taxi.utils.representations import (
    json_to_image,
    json_to_discrete,
    json_to_discrete_predictable,
    resize_image,
    json_to_screen,
    json_to_mixed,
    json_to_mlp,
...
from domains.gym_taxi.utils.config import (
    MAP,
    MAX_EPISODE_LENGTH,
    LOCS,
...
class BoxTaxiEnv(FixedTaxiEnv):
    ...
    def step(self, action):
        ...
        return resize_image(json_to_image(obs), OUTPUT_IMAGE_SIZE), reward, done, info

    def reset(self):
        return resize_image(json_to_image(super().reset()), OUTPUT_IMAGE_SIZE)
...
class JsonTaxiEnv(BaseTaxiEnv):
    ...
    def _get_random_action(self, obs):
        ...
        if location:
            x, y = np.random.randint(0, size, 2)  # TODO: magic number
            action["location"] = (x, y)
        if delivered:
            action["delivered"] = [
...
def create_network(env):
    ...
    network.add_nodes_from([tuple(n) for n in env["network"]["nodes"]])
    network.add_edges_from([(tuple(x), tuple(y)) for x, y in env["network"]["edges"]])
    return network


def find_path(network, start, end):
    path = nx.astar_path(network, start, end)
...
def translate_path(path):
    ...
    x2, y2 = path[0]
    for x1, y1 in path[1:]:
        actions.append(DIRECTIONS[(x1 - x2, y1 - y2)])
        x2, y2 = x1, y1
    return actions


# TODO: These are duplicates of code in the above classes, and should really not be here. This is to allow it to be used globally during multithreading, which is a massive hack
def generate_pddl(ob, subgoal):
    pddl = json_to_pddl(ob)
    pddl = pddl.replace("$goal$", _action_to_pddl(subgoal))
    pddl = pddl.replace("$nodes$", " ".join(list(set(re.findall("sx\d+y\d+", pddl)))))
    return pddl


def _action_to_pddl(action):
    goal = ""
    goal += "(empty t)\n" if action["empty"] else ""
    ...
    if action["delivered"] is not None:
        goal += "".join([f"(delivered p{p})\n" for p in action["delivered"]])
    if action["passenger"] is not None:
        goal += "".join([f"(carrying-passenger t p{p})\n" for p in action["passenger"]])
    if action["location"] is not None:
        x, y = action["location"]
        goal += f"(in t sx{x}y{y})\n"
...
def expand_actions(obs, actions):
    if not any([a.startswith("move ") for a in actions]):
        return [(a, None) for a in actions]  # to fit craft action signiature
